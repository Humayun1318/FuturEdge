...
import { FaRegChartBar, FaLightbulb, FaUsers, FaArrowUp, FaDownload } from 'react-icons/fa';
...
const USPDetails = () => {
  ...
  useEffect(() => {
    ...
    if (data?.USP.length) {
      const selectedCard = data?.USP.find((card) => Number(card.id) === Number(id))
        ...
        <div className="bg-gradient-to-r from-blue-800 to-blue-600 p-6">
          <div className="flex items-center space-x-4">
            <img
              ...
              loading="lazy"
            />
            <h1 className="text-3xl font-bold text-white font-lora-heading">{title}</h1>
          </div>
        </div>
